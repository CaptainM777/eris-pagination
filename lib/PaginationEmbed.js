...
class PaginationEmbed {
    ...
    constructor(message, pages = [], options = {}, components) {
        ...
        this.isInteraction = !!this.invoker.interaction;
    }
    ...
    async initialize() {
        ...
        this.handler = new ReactionHandler.continuousReactionStream(this.message, (userID) => userID === (this.isInteraction ? this.invoker.interaction.member.id : this.invoker.author.id), false, { maxMatches: this.maxMatches, time: this.timeout });
        ...
        this.handler.on('reacted', async (event) => {
            switch (event.emoji.name) {
                case this.firstPage: {
                    if (this.advanced) {
                        if (this.checkPerms()) {
                            await this.message.removeReaction(this.firstPage, (this.isInteraction ? this.invoker.interaction.member.id : this.invoker.author.id));
                        }
                ...
                case this.back: {
                    if (this.checkPerms()) {
                        await this.message.removeReaction(this.back, (this.isInteraction ? this.invoker.interaction.member.id : this.invoker.author.id));
                    }
                ...
                case this.forth: {
                    if (this.checkPerms()) {
                        await this.message.removeReaction(this.forth, (this.isInteraction ? this.invoker.interaction.member.id : this.invoker.author.id));
                    }
                ...
                case this.lastPage: {
                    if (this.advanced) {
                        if (this.checkPerms()) {
                            await this.message.removeReaction(this.lastPage, (this.isInteraction ? this.invoker.interaction.member.id : this.invoker.author.id));
                        }
